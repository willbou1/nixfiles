...
with builtins; let
  ...
  expressvpnServers = servers:
    builtins.listToAttrs (map (s: {
        name = s.name;
        value = {
          config = ''            config /etc/openvpn/expressvpn/my_expressvpn_${strings.stringAsChars (c:
              if c == " "
              then "_"
              else c)
...
in {
  ...
  config = {
    ...
    environment.etc."test".text = toJSON (evalRuleset (with notnft.dsl;
      with payload;
        ruleset {
