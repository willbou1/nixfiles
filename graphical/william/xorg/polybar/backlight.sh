...
gamma="$(cat ~/.config/polybar/gamma)"
...
case $BUTTON in
	...
	2)
		...
		xrandr $(xrandr --listmonitors  | awk -v bri="$brightness" -v gam="$gamma" '!(/Monitors/) {printf "--output %s --gamma %s --brightness %f ", $NF, gam, bri}') &
                ;;
        3)
        import png:- | xclip -i -selection clipboard -target image/png
                ;;
esac
disown -a

echo $gamma > ~/.config/polybar/gamma
echo $brightness > ~/.config/polybar/brightness
