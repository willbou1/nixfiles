...
(defcustom feebleline-msg-functions
	...
	(feebleline-file-mode           :pre " " :face font-lock-escape-face)
...
(defface feebleline-major-mode-face '((t :inherit 'font-lock-variable-name-face))
  "Example face for dir face."
  :group 'feebleline)
...
(defun feebleline-file-owner ()
  ...
  (when buffer-file-name
    (let ((attributes (file-attributes buffer-file-name)))
      (concat (number-to-string (file-attribute-user-id attributes)) " "
	      (number-to-string (file-attribute-group-id attributes))))))
